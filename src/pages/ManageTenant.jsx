      ...
      try {
        ...
        if (dataConn.success && dataConn.data) {
          const norm = dataConn.data.map(c => ({
            ...
            loginId: c.login_id,
            password: c.password,
            bindType: c.bind_type || '',
            smsThroughput: c.sms_throughput || '100',
            bufferSize: c.buffer_size || '1000',
            timeout: c.timeout || '30',
            retryCount: c.retry_count || '3',
            keepAliveInterval: c.keep_alive_interval || '60',
            keepAliveEnabled: !!c.keep_alive_enabled,
            description: c.description || '',
            dedicatedConnection: !!c.dedicated_connection,
            internationalAllowed: !!c.international_allowed,
            roundRobinEnabled: !!c.round_robin_enabled,
            mtShortcode: c.mt_shortcode || '',
            moShortcode: c.mo_shortcode || '',
            modifiedDate: c.modified_date ? c.modified_date.replace('T', ' ').substring(0, 19) : c.modifiedDate,
            modifier: c.modifier
          }));
          setConnections(norm);
      ...
      try {
        ...
        if (dataTenant.success && dataTenant.data) {
          const norm = dataTenant.data.map(t => ({
            ...
            password: t.password,
            moCertificateName: t.mo_certificate_name || 'None',
            moAccessProtocol: t.mo_access_protocol || '',
            sslEnabled: !!t.ssl_enabled,
            bounceSmsUrl: t.bounce_sms_url || '',
            moProductionUrl: t.mo_production_url || '',
            dlrStagingUrl: t.dlr_staging_url || '',
            dlrProductionUrl: t.dlr_production_url || '',
            mtStagingUrl: t.mt_staging_url || '',
            mtProductionUrl1: t.mt_production_url1 || '',
            mtProductionUrl2: t.mt_production_url2 || '',
            cpPortalUrl: t.cp_portal_url || '',
            localRoutingOperator: t.local_routing_operator || '',
            internationalRoutingOperator: t.international_routing_operator || '',
            description: t.description || 'None',
            mtService: !!t.mt_service,
            moService: !!t.mo_service,
            moConcatenation: !!t.mo_concatenation,
            tpoa: !!t.tpoa,
            alternateRoute: !!t.alternate_route,
            deliveryReceipt: !!t.delivery_receipt,
            legacySupport: !!t.legacy_support,
            returnDeliveryReceipt: !!t.return_delivery_receipt,
            dedicatedService: !!t.dedicated_service,
            divertPortedOutNumber: !!t.divert_ported_out_number,
